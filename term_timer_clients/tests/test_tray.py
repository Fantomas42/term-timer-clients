...
class StubPopup(Popup):
    """
    A window that is opened and closed without a process behind it.
    ...
    def settle(self) -> bool:
        """
        Notice a window that closed itself.
    ...
    def test_the_window_is_opened_on_the_stream_of_the_icon(self) -> None:
        ...
        self.assertEqual(
            ...
            ['cast', '--endpoint', 'ipc:///tmp/cube',
             TRANSPARENT_FLAG, '--window-size', '100x200'],
        )
...
class PopupTestCase(unittest.TestCase):
    """The window as the process it is."""
    ...
    def test_a_popup_opens_nothing_by_itself(self) -> None:
        """A tray starts with an icon and no window."""
    ...
    def test_a_second_click_opens_no_second_window(self) -> None:
        """One icon, one window."""
        with patch(
                'term_timer_clients.tray.cast.subprocess.Popen',
        ) as opening:
    ...
    def test_a_window_that_is_still_up_is_not_settled(self) -> None:
        ...
        with patch(
            ...
            opening.return_value.poll.return_value = None
            self.popup.show()

            self.assertFalse(self.popup.settle())
            self.assertTrue(self.popup.shown)

    def test_a_window_closed_by_hand_is_noticed(self) -> None:
        """Q closes the window, and the icon has to hear about it."""
        with patch(
                'term_timer_clients.tray.cast.subprocess.Popen',
        ) as opening:
            opening.return_value.poll.return_value = 0
            self.popup.show()

            self.assertTrue(self.popup.settle())
            self.assertFalse(self.popup.shown)

    def test_a_window_that_is_not_up_is_not_closed(self) -> None:
        """Nothing is signalled where there is no process."""
        self.popup.hide()

        self.assertFalse(self.popup.shown)

    def test_a_second_click_closes_the_window(self) -> None:
        ...
        opening.return_value.wait.assert_called()
        self.assertFalse(self.popup.shown)
    ...
    def test_a_window_that_will_not_go_is_taken_down(self) -> None:
        ...
        with patch(
            ...
            process.wait.side_effect = [
                subprocess.TimeoutExpired('cube-cast', 2.0), None,
            ]
    ...
    def test_a_window_is_asked_the_way_a_terminal_asks(self) -> None:
        """It closes through the path it closes by hand."""
        with patch(
                'term_timer_clients.tray.cast.subprocess.Popen',
        ) as opening:
            process = opening.return_value
