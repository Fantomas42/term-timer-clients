...
# The hardware plane, and the two messages of the other one a
...
# is doing is what says a scramble was laid on the cube and an attempt
# is over, which no cube can tell - a solve and a trained case end on
# the very same stop, and a cube reports itself solved whenever it
# happens to be. Everything else term-timer knows alone stays on the
# wire.
PREFIXES = (
    CUBE_PREFIX,
    SESSION_STATE_TOPIC,
