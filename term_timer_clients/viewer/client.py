...
# The two states a session is in at the two ends of an attempt, and
# the only ones the window has anything to say about: the scramble laid
# on the cube, and the attempt over. **The stop is term-timer's and not
# the cube's**: it is published at the end of every attempt, a solve or
# a trained case, whichever cube is connected and whether or not the
# attempt came out - a solve stopped from the keyboard included. What
# the cube says of itself does not do: only some drivers report it
# solved at all, and those that do report it at every return to the
# solved state, scrambling and idle handling included.
...
class CubeCast(CubeLink):
    ...
    def follow_state(self, data: dict[str, Any]) -> None:
        """
        Greet the two ends of an attempt, and let the rest go by.

        One breath per attempt, and the same one whichever way it
        went: the window says an attempt is over, not whether it was
        worth anything - that is what term-timer keeps of it.

        A state this client has never heard of is simply ignored: the
        nine of the protocol may grow, and a window refusing what it
        does not know would stop answering the day one is added.
